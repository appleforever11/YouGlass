...
- The full-player overlay is ambient-backdrop-first: keep the watch page fills translucent, let the shared backdrop flow through the faded header/page/recommendation rail, and clip only the media content so its glow is not cut off. The watch page has one outer vertical scroll owner; comments and Up Next stay intrinsic-height and must not regain competing vertical scroll views. Keep the media/WebKit/native boundaries on the shared radius and use centered perimeter shadows. Route video-opening button actions through the deferred user-interaction helper on macOS 26+.
