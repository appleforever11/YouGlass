...
- The full-player overlay is ambient-backdrop-first: keep the watch page fills translucent, let the shared backdrop flow through the header/page/recommendation rail, and clip only the media content so its glow is not cut off. Route video-opening button actions through the deferred user-interaction helper on macOS 26+.
