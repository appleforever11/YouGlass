...
- Desktop discovery surfaces share one interaction contract: Home uses the YouGlass app identity, `Command-L` requests focus for global search, Settings filters pages by tokenized feature keywords without mutating preferences, and sidebar/card controls expose visible pointer feedback plus accessibility labels. Home's single vertical scroll owner also provides native pull/drag-down refresh for the normal Home feed and routes it through the existing quota-safe forced load; on macOS 27+ use the AppKit `NSRefreshController` bridge, with SwiftUI `.refreshable` as the older-system fallback. Library, Search, and focused Custom Feeds keep their own refresh boundaries. Decorative indicators must represent a real action or state; do not reintroduce inert carousel dots.
...
- Home recommendation freshness uses a quota-safe foreground cadence: automatic refresh remains once per minute, while deliberate refreshes bypass the short-lived API response cache, refresh account signals/subscriptions, and favor candidates outside the bounded recently-presented window. Store only recommendation IDs for that rotation state; never store credentials or raw account data for it.
