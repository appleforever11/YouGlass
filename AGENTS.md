...
- Desktop discovery surfaces share one interaction contract: Home uses the YouGlass app identity, `Command-L` requests focus for global search, Settings filters pages by tokenized feature keywords without mutating preferences, and sidebar/card controls expose visible pointer feedback plus accessibility labels. Decorative indicators must represent a real action or state; do not reintroduce inert carousel dots.
