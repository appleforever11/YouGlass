...
- Normal-player transport chrome starts hidden, reveals on media hover or interaction, fades after pointer exit, and moves the native caption overlay with the same visibility state; compact/PIP uses the same transient visibility contract for its top window shelf and bottom transport, with hidden controls relinquishing hit testing so the player remains draggable.
