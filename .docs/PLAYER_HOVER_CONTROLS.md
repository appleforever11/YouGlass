...
The normal expanded player keeps its native transport chrome out of the resting frame. The player root owns pointer hover, so the transport appears when the pointer enters the media surface and remains available while the pointer is over the player. A short 0.55-second exit grace period prevents the controls from blinking while the pointer crosses the media edge; after that, the controls and their bottom gradient fade out. Explicit transport interactions still reveal the controls and use the existing delayed hide fallback when the pointer is not over the player.
